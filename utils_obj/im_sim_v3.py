# to preprocess image
# yolo passes the array and the image is not created. The array is processed, so the process is faster.
...
rust_weight = 'weights/rustnorust_model_xception.h5'
rust_model  = keras.models.load_model(rust_weight)
def rust_classifier(im, nbox):
    x0 = int(nbox[0]*W)
    y0 = int(nbox[1]*H)
    h2 = int(nbox[3]*H / 2)
    w2 = int(nbox[2]*W / 2)
    img = im[y0 - h2: y0 + h2, x0 - w2:x0 + w2, :]
    # im is a numpy array
    test_x = cv2.resize(img, dsize=(150, 150), interpolation=cv2.INTER_CUBIC)

    # Reshape it to (1, 150, 150, 3)
    test_x = test_x.reshape((1,) + test_x.shape)
    test_x = test_x.astype('float32') / 255
    rust_prob = rust_model.predict(test_x)
...
def extract_features(file, model = model):
    ...
    test_x = file.reshape((1,) + file.shape)
    test_x = test_x.astype('float32') / 255

    # imgx = preprocess_input(test_x)
    # get the feature vector
    features = model.predict(test_x, use_multiprocessing=True)
    return features
...
class Sim(object):
    ...
    def info(self, fps, save_dir, width, height):
        global H, W
        H = height
        W = width
        self.fps = fps
    ...
    def new_im(self, im, frame):
        ...
        if self.current_frame - self.last_simil >= self.skip*self.fps:
            ...
            try:
                ...
                self.nim = cv2.resize(im, dsize=(299, 299), interpolation=cv2.INTER_CUBIC)
                new_vec = extract_features(self.nim, model)
